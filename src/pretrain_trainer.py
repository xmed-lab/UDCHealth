...
from metrics import multilabel_metrics_fn, regression_metrics_fn

from pyhealth.utils import create_directory
from config import config
from utils import plot_tsne, adjust_learning_rate, merge_dicts
...
class PTrainer:
    ...
    def inference(self, dataloader, additional_outputs=None,
        ...
        for data in tqdm(dataloader, desc="Evaluation"):
            ...
            with torch.no_grad():
                output = self.model(**data)
                loss = output["loss"]
                pcf_embedding_loss = output["pcf_embedding_loss"] # commitment loss
                plm_embedding_loss= output["plm_embedding_loss"]
                pcf_recon_loss= output["pcf_recon_loss"]
                plm_recon_loss= output["plm_recon_loss"]
                task_aware_loss= output["task_aware_loss"]
                loss_emb_pcf.append(pcf_embedding_loss.item())
                loss_emb_plm.append(plm_embedding_loss.item())
                loss_recon_pcf.append(pcf_recon_loss.item())
                loss_recon_plm.append(plm_recon_loss.item())
                loss_task_aware.append(task_aware_loss.item())

                y_pcf_true = output["y_pcf_true"].cpu().numpy()
                y_pcf_prob = output["y_pcf_prob"].cpu().numpy()
                y_plm_true = output["y_plm_true"].cpu().numpy()
                y_plm_prob = output["y_plm_prob"].cpu().numpy()
                loss_all.append(loss.item())
                y_pcf_true_all.append(y_pcf_true)
                y_pcf_prob_all.append(y_pcf_prob)
                y_plm_true_all.append(y_plm_true)
                y_plm_prob_all.append(y_plm_prob)
        ...
        loss_emb_pcf_mean = sum(loss_emb_pcf) / len(loss_emb_pcf)
        loss_emb_plm_mean = sum(loss_emb_plm) / len(loss_emb_plm)
        loss_recon_pcf_mean = sum(loss_recon_pcf) / len(loss_recon_pcf)
        loss_recon_plm_mean = sum(loss_recon_plm) / len(loss_recon_plm)
        loss_task_aware_mean = sum(loss_task_aware) / len(loss_task_aware)

        y_pcf_true_all = np.concatenate(y_pcf_true_all, axis=0)
        y_pcf_prob_all = np.concatenate(y_pcf_prob_all, axis=0)
        y_plm_true_all = np.concatenate(y_plm_true_all, axis=0)
        y_plm_prob_all = np.concatenate(y_plm_prob_all, axis=0)
        outputs = [y_pcf_true_all, y_pcf_prob_all, y_plm_true_all, y_plm_prob_all, loss_mean, (loss_emb_pcf_mean, loss_emb_plm_mean, loss_recon_pcf_mean, loss_recon_plm_mean, loss_task_aware_mean)]
        if additional_outputs is not None:
    ...
    def evaluate(self, dataloader, aux_data=None) -> Dict[str, float]:
        """Evaluates the model.
        ...
        if self.model.mode is not None: # mode指定了metric
            y_pcf_true_all, y_pcf_prob_all, y_plm_true_all, y_plm_prob_all, loss_mean, aux_loss = self.inference(dataloader, additional_outputs=None)
            mode = self.model.mode
            metrics_fn = get_metrics_fn(mode)
            scores_pcf = metrics_fn(y_pcf_true_all, y_pcf_prob_all, metrics=self.metrics, aux_data=aux_data) # pcf mse loss
            scores_plm = metrics_fn(y_plm_true_all, y_plm_prob_all, metrics=self.metrics, aux_data=aux_data)
            scores = merge_dicts(scores_pcf, scores_plm)
            scores["loss"] = loss_mean
            scores["aux_loss_grouped"] = aux_loss # (emb_pcf_mean, emb_plm_mean, recon_pcf_mean, recon_plm_mean, task_aware_mean)
        else: # 主要是改这里，接上inference
            loss_all = []
            for data in tqdm(dataloader, desc="Evaluation"):
                self.model.eval()
                with torch.no_grad():
                    output = self.model(**data)
                    loss = output["loss"] # 这里得改；
                    loss_all.append(loss.item())
            loss_mean = sum(loss_all) / len(loss_all)
            scores = {"loss": loss_mean}
        return scores
